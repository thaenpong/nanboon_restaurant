...
if (isset($_GET["fromDate"]) && isset($_GET["toDate"])) {
    ...
    $sql = "SELECT menus.name, 
            ...
            SUM(orderitems.discount * orderitems.quantity) AS discount, 
            SUM(menus.price * orderitems.quantity) AS total_menu_price, 
            SUM((menus.price * orderitems.quantity) - (orderitems.discount * orderitems.quantity)) AS total 
            FROM orderitems
    ...
    $totalDiscount = 0;
    $menusData = [];
    while ($row = mysqli_fetch_assoc($qrTotal)) {
    ...
    $pdf->Cell(1, 8, iconv('UTF-8', 'cp874', 'วันที่ : ' . $fromDate . ' - ' . $toDate), 0, 0, 'r');
    $pdf->Ln();
